...
def load_params():
    ...
    params = loadmat(os.path.join(dir_path, 'decodingModel_01_para.mat'))
    x1_step1 = params["x1_step1"]
    new_x1_step1 = {}
    new_x1_step1["xoffset"] = x1_step1["xoffset"].item().flatten().astype(float)
    new_x1_step1["gain"] = x1_step1["gain"].item().flatten().astype(float)
    new_x1_step1["ymin"] = x1_step1["ymin"].item().item()
